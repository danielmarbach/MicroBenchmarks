...
namespace MicroBenchmarks.Buffers
{
    ...
    public class DictionaryBuffers
    {
        ...
        public void Dictionary_Allocate() 
            => DeadCodeEliminationHelper.KeepAliveWithoutBoxing(new Dictionary<string, string>[Size]);

        [Benchmark]
        public void Task_RentAndReturn_Aware()
        {
            var pool = dictionaryBuffer;
            var dictionary = pool.Rent(Size);
        ...
        class DeadSimpleDictionaryBuffer
        {
            ...
            public Dictionary<string, string> Rent(int size)
            {
                ...
                {
                    case 1:
                        buffer1.TryTake(out dictionary);
                        break;
                    case 2:
                        buffer2.TryTake(out dictionary);
                        break;
                    case 8:
                        buffer8.TryTake(out dictionary);
                        break;
                    case 16:
                        buffer16.TryTake(out dictionary);
                        break;
                    case 32:
                        buffer32.TryTake(out dictionary);
                        break;
                    case 64:
                        buffer64.TryTake(out dictionary);
                        break;                    
                }
