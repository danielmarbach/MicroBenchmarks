...
public static class TrampolineUnsafeOnCompleted
{
...
#pragma warning restore PS0019
            ...
            if (!task.IsCompleted)
            {
                return RestoreWithUnsafeOnCompleted(task, context, frame);
            }
        ...
        [StackTraceHidden]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static Task RestoreWithUnsafeOnCompleted(Task task, Trampoline.BehaviorContext ctx, Trampoline.PipelineFrame frame)
        {
            var state = new RestoreState(task.GetAwaiter(), ctx, frame);
            state.Awaiter.UnsafeOnCompleted(state.Continuation);
            return state.Promise.Task;
        }

        private sealed class RestoreState(TaskAwaiter awaiter, Trampoline.BehaviorContext context, Trampoline.PipelineFrame frame)
        {
            public readonly TaskAwaiter Awaiter = awaiter;
            public readonly Trampoline.BehaviorContext Context = context;
            public readonly Trampoline.PipelineFrame Frame = frame;
            public readonly TaskCompletionSource<object?> Promise = new(TaskCreationOptions.None);
