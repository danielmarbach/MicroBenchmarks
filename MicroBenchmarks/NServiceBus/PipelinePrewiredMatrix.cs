...
public class PipelinePrewiredMatrix
{
    CurrentBehaviorContext currentContext;
    PipelineAfterOptimizationsUnsafeAndMemoryMarshal<IBehaviorContext> currentSuccessPipeline;
    PipelineAfterOptimizationsUnsafeAndMemoryMarshal<IBehaviorContext> currentExceptionPipeline;
    PipelineAfterOptimizationsUnsafeAndMemoryMarshal<IBehaviorContext> currentSyncExceptionPipeline;
    PipelineAfterOptimizationsUnsafeAndMemoryMarshal<IBehaviorContext> currentReplayPipeline;

    Trampoline.BehaviorContext trampolineSuccessContext;
    Trampoline.BehaviorContext trampolineExceptionContext;
    Trampoline.BehaviorContext trampolineSyncExceptionContext;
    Trampoline.BehaviorContext trampolineReplayContext;
    ...
    public void Setup()
    {
        ...
        prewiredSuccessContext = CreateTrampolineSuccessContext(PipelineDepth);
        prewiredExceptionContext = CreateTrampolineExceptionContext(PipelineDepth);
        prewiredSyncExceptionContext = CreateTrampolineSyncExceptionContext(PipelineDepth);
        prewiredReplayContext = CreateTrampolineReplayContext(PipelineDepth, ReplayCount);

        currentSuccessPipeline.Invoke(currentContext).GetAwaiter().GetResult();
        Trampoline.StageRunners.Start(trampolineSuccessContext).GetAwaiter().GetResult();
        Prewired.StageRunners.Start(prewiredSuccessContext).GetAwaiter().GetResult();
    }

    [Benchmark(Baseline = true), BenchmarkCategory("Success")]
    public Task Current_Success() => currentSuccessPipeline.Invoke(currentContext);

    [Benchmark, BenchmarkCategory("Success")]
    public Task Trampo_Success() => Trampoline.StageRunners.Start(trampolineSuccessContext);

    [Benchmark, BenchmarkCategory("Success")]
    public Task Prewired_Success() => Prewired.StageRunners.Start(prewiredSuccessContext);
    ...
    public Task Trampo_Replay() => Trampoline.StageRunners.Start(trampolineReplayContext);
    ...
    public async Task<Exception?> Trampo_Exception()
    {
        try
        {
            await Trampoline.StageRunners.Start(trampolineExceptionContext).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            return ex;
    ...
    public async Task<Exception?> Trampo_Exception_Sync()
    {
        try
        {
            await Trampoline.StageRunners.Start(trampolineSyncExceptionContext).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            return ex;
