...
public static class TrampolineValueTaskSource
{
...
#pragma warning restore PS0019
            ...
            if (!task.IsCompleted)
            {
                return RestoreFrameValueTaskSource.Create(task, context, frame);
            }
    ...
    sealed class RestoreFrameValueTaskSource : IValueTaskSource
    {
        static readonly ConcurrentBag<RestoreFrameValueTaskSource> Pool = new();

        ManualResetValueTaskSourceCore<bool> core;
        TaskAwaiter awaiter;
        Trampoline.BehaviorContext? context;
        Trampoline.PipelineFrame frame;
        readonly Action continuation;

        RestoreFrameValueTaskSource()
        {
            core.RunContinuationsAsynchronously = false;
            continuation = Continuation;
        }

        public static Task Create(Task pendingTask, Trampoline.BehaviorContext context, Trampoline.PipelineFrame frame)
        {
            if (!Pool.TryTake(out var source))
            {
                source = new RestoreFrameValueTaskSource();
            }

            source.core.Reset();
            source.context = context;
            source.frame = frame;
            source.awaiter = pendingTask.GetAwaiter();
            source.awaiter.UnsafeOnCompleted(source.continuation);

            return new ValueTask(source, source.core.Version).AsTask();
        }
        ...
        void Continuation()
        {
            ...
            {
                // Must restore before completing core to avoid synchronous reentrancy
                // via core.SetResult/SetException calling GetResult() which clears context.
                context!.Frame = frame;
            }
